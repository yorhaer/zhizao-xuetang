    ...
    <aside class="sidebar">
      <div class="sidebar-brand">
        ...
        <div class="brand-text">
          ...
          <span class="brand-role">管理者看板</span>
        </div>
      </div>

      <nav class="sidebar-nav">
        <router-link to="/dashboard/overview" class="nav-item" active-class="nav-item--active">
          <span class="nav-icon">🔮</span>
          <span class="nav-label">总览</span>
        </router-link>
        <router-link to="/dashboard/comparison" class="nav-item" active-class="nav-item--active">
          <span class="nav-icon">📈</span>
          <span class="nav-label">工作室对比</span>
        </router-link>
      </nav>

      <div class="sidebar-footer">
        <div class="user-card">
          <div class="user-avatar manager-avatar">{{ userStore.userName?.charAt(0) }}</div>
          <div class="user-info">
...
.sidebar {
  ...
  background: linear-gradient(180deg, #0c1826 0%, #112036 100%);
  display: flex; flex-direction: column; box-shadow: var(--shadow-sidebar); z-index: 100;
}
...
.brand-role { font-size: 11px; color: #64748b; margin-top: 1px; }
...
.nav-item:hover { background: rgba(255,255,255,0.06); color: #cbd5e1; border-left-color: rgba(99,102,241,0.4); }
.nav-item--active { background: rgba(99,102,241,0.18); color: #a5b4fc; border-left-color: #6366f1; font-weight: 600; }
.nav-icon { font-size: 16px; width: 20px; text-align: center; flex-shrink: 0; }

.sidebar-footer { padding: 12px; border-top: 1px solid rgba(148,163,184,0.1); }
...
.manager-avatar { background: linear-gradient(135deg, #7c3aed, #8b5cf6); }
...
.top-title { font-size: 16px; font-weight: 700; color: var(--text-primary); }
